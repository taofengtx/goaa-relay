...
樣式（切分書寫）：`"sk_" + "live_"`、`"sk_" + "test_"`、`"BEGIN " + "PRIVATE KEY"`、`"AK" + "IA"`、`"gh" + "p_"`、`"postgres" + ":" + "//"`、`"PGPASSWORD" + "="`、`"pass" + "word="`、`"ey" + "J"`、`".pgp" + "_m"`。
...
**唯一 1 筆非零命中＝暫存檔名**：樣式 `".pgp" + "_m"` 命中 1 處，位置為步驟 2 預定指令中的 **`PGPASSFILE=/tmp/.pgp_m`**（即 `-e PGPASSFILE=<路徑>`）。**該處只是暫存檔「路徑」，不含任何密碼值、雜湊或金鑰片段**，且檔案於使用後即刪。⇒ **實質機密命中數仍為 0。**

**未切分 IPv4 掃描**：**0 命中**（IPv4 一律以 `⟨N⟩` 切分書寫）。

**檔案完整性**：`REPORT.md` = **7,526 bytes**、`sha256` 前16 = `759bc4db880f1d0b`、首三 byte = `b'# R'`（**無 BOM**）。

**relay main sha**：`a31d55868ebb38a8fbed51dbc048296d40fe1841`
（本報告**內容** commit；其後的子提交僅用於寫入本行與上方掃描回報，未改動任何校驗結論。）
