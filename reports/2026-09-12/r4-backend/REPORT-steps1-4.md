...
- `"." + "pgp" + "ass"` **5 處** —— 全部是**檔名／路徑**（`app.` 加 `pgpass`、`/opt/goaa-platform/env/` 加 `app.` 加 `pgpass` 等）。**不含任何密碼內容**。
- `"clerk" + "_secret"` **1 處** —— 是**變數名稱**（`CLERK_` 加 `SECRET_KEY`）出現在 §3.4「其餘照抄（17 鍵）」清單中。**不是鍵值本身**；本報告自始至終未讀取、未輸出該值。

**IPv4 書寫**：**可路由（公開）位址未切分命中數 = 0**；`127.0.0.1`（loopback）與 `0.0.0.0`（unspecified）依既有報告慣例**逐字書寫**（非可路由、非機密）。

**秘密處理**：DB 應用密碼與 `GOAA_C2_SESSION_SECRET` **全程未回顯、未進報告、未上命令列**；報告只列**長度**與 **sha256 前 16 位**。C2 env 原文以 `umask 077` 直接落為 C1 上的 `0600` 中繼檔，未經對話。

**檔案完整性（本節追加前，＝內容 commit 之版本）**：

| 檔案 | bytes | sha256[:16] | 首三 byte | BOM |
|---|---|---|---|---|
| `REPORT-steps1-4.md` | 15,952 | `00b3b411f63fd7d7` | `b'# R'` | 無 |
| `RECON.md`（含 §勘誤） | 33,005 | `53a32d07087f042d` | `b'# R'` | 無 |

**relay main sha**：`f1736fa`（本報告**內容** commit；其後子提交僅追加本節與本行，未改動任何驗收結論。）
