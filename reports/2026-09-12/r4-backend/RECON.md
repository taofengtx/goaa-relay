...
## 勘誤（2026-09-12，於 R4 步驟 1–4 執行時發現）

**位置**：§A1b 刊出的「樹摘要 `81b17744f8134956`」。

**性質**：**報告瑕疵（雜湊輸入多算了一行），不是內容差異。**

**根因**：當時計算樹摘要的正規化程序只濾掉 `---FILE_COUNT---` 這行標記，卻把緊接其後的**檔數統計行 `37`** 一併納入雜湊輸入（該行首字元 `3` 落在所選的十六進位字元集內，因此未被濾除）。故刊出值實際上是「37 行 sha256 清單 ＋ 一行 `37`」的雜湊。

**重算（同一份 `/tmp/r4-c2-manifest.txt`）**：

| 算法 | sha256 前 16 |
|---|---|
| 含瑕疵行（＝原刊出值） | `81b17744f8134956` |
| 乾淨 37 行 | `bbb5c0bf45d88da1` |

**實質結論不變**：該 37 檔的逐位元比對（同報告 §A1b 所述之 `r4-cmp.py`）為 **37/37 相同、單邊 0/0、內容不同 0**。2026-09-12 於 C1 重建時，兩端以同一支程序各算一次，皆得 `bbb5c0bf45d88da1`，再次確認兩者為同一棵樹。

**處理**：本節為附錄式補正；**不回改原刊出之數字**，以保留歷史原貌。後續引用請以 `bbb5c0bf45d88da1` 為準。詳見同目錄 `REPORT-steps1-4.md` §1.4。
