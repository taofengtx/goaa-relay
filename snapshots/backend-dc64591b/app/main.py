...
from . import ai_review, audit, clerk_auth, clerk_identity, golden_session, storage
...
def _clerk_user(settings: Settings, authorization: str | None) -> dict[str, Any]:
    """Clerk path: verify the session token independently, then map the subject.

    Nothing the front-end claims about the caller is trusted here — only the
    token itself. Roles are read from the local database afterwards, so a
    Clerk account can never grant itself ``agent``/``admin``.
    """
    try:
        identity = clerk_auth.verify_session_token(authorization, settings)
    except clerk_auth.ClerkAuthError as exc:
        raise _HTTPError(exc.status, exc.code, exc.message)

    def verified_email() -> str | None:
        """The verified primary address, read back from Clerk server-to-server.

        This is deliberately lazy: it is only ever called when ``(issuer,
        subject)`` has no local mapping yet, so an already-known caller never
        costs a Clerk Backend API round trip on each request. A lookup failure
        simply leaves the local address NULL.
        """
        return clerk_auth.verified_primary_email(settings, identity.subject)
    ...
    try:
        user = clerk_identity.resolve_clerk_user(
            ...
            email_provider=verified_email,
        )
    except clerk_identity.IdentityConflict:
        raise _HTTPError(
            409,
            "identity_conflict",
            "this identity cannot be linked to an existing account",
        )
    except clerk_identity.IdentityError:
        user = None

    if not user:
        raise _HTTPError(401, "invalid_clerk_session", "the clerk identity could not be resolved")
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def current_user(
        ...
        if cfg.clerk_auth_enabled:
            # Clerk is the identity authority: the browser's session token is
            # the only accepted proof. No cookie, no BFF assertion.
            return _clerk_user(cfg, authorization)
        # --- legacy session path (Golden-Flow rollback, unchanged) ---------
        token = None
    ...
    async def _no_store(request: Request, call_next):
        ...
        if settings.clerk_auth_enabled and request.url.path.rstrip("/") in legacy_auth_paths:
            response = _error(409, "legacy_auth_disabled", "use Clerk to manage authentication")
        else:
    ...
    #
    # The browser holds only the Clerk session; the BFF forwards that same
    # session token here, and `current_user` re-verifies it independently.
    # `(issuer, subject)` is then linked to a business subject and an opaque,
    # revocable business token is minted. Nothing about the caller is read from
    # the request body, and the C2 *test* `users` id is never reused as a
    # golden business id.
    def _golden_identity(user: dict[str, Any]) -> tuple[str, str, str]:
        """(local user id, issuer, subject) — Clerk only; fail closed otherwise."""

        if not settings.clerk_auth_enabled:
            # Without Clerk as the identity authority there is no browser-proven
            # identity to bridge, and a legacy session cookie must never be
            # promoted into a golden business credential.
            raise _HTTPError(409, "clerk_auth_disabled", "the golden identity bridge requires Clerk")
        issuer = str(user.get("identity_issuer") or "")
        subject = str(user.get("identity_subject") or "")
        if not issuer or not subject:
            raise _HTTPError(401, "invalid_session", "the clerk identity could not be resolved")
        return str(user["user_id"]), issuer, subject

    @app.get(f"{API_PREFIX}/golden/session")
    def golden_session_status(user: dict[str, Any] = Depends(current_user)) -> dict[str, Any]:
        """Whether this identity is already linked, and which session is live."""

        user_id, _issuer, _subject = _golden_identity(user)
        with connection(settings) as conn:
            linked = golden_session.subject_for_user(conn, user_id)
            if linked is None:
                return {"provider": "clerk", "linked": False, "session": None}
            business_user_id = str(linked["subject_id"])
            live = golden_session.list_live_tokens(conn, business_user_id)
            return {
                "provider": "clerk",
                "linked": True,
                "session": {
                    # The business principal id, in the golden contract's own
                    # vocabulary. `live_tokens` is a count only: no token value
                    # is ever read back out of the database.
                    "user_id": business_user_id,
                    "role": golden_session.BUSINESS_ROLE,
                    "live_tokens": len(live),
                },
            }

    @app.post(f"{API_PREFIX}/golden/session")
    def golden_session_issue(user: dict[str, Any] = Depends(current_user)) -> JSONResponse:
        """Link once (by ``(issuer, subject)``) and mint one fresh opaque token.

        The response body is the *only* place the plaintext token ever appears;
        it is for the BFF, which puts it in an httpOnly cookie. It is never
        logged and never stored in plaintext anywhere.
        """

        user_id, issuer, subject = _golden_identity(user)
        with connection(settings) as conn:
            ...
            issued = golden_session.issue_token(
                conn,
                user_id=user_id,
                issuer=issuer,
                subject=subject,
    ...
    @app.get(f"{API_PREFIX}/golden/session/verify")
    def golden_session_verify(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        """Resolve the *business* credential — used on refresh, and for calls.

        This is the second half of the bridge: the Clerk session proves who the
        human is (once), and this opaque, database-backed, revocable token is
        what the business surface then accepts. It carries no identity of its
        own: the subject, role and expiry are all read back from the row, and a
        revoked or expired row is refused. The token is never echoed.
        """

        if not settings.clerk_auth_enabled:
            # The bridge only exists while Clerk is the identity authority.
            raise _HTTPError(409, "clerk_auth_disabled", "the golden identity bridge requires Clerk")
        token = _bearer_token(authorization)
        if not golden_session.looks_like_token(token):
            raise _HTTPError(401, "invalid_business_token", "a business token is required")
        try:
            with connection(settings) as conn:
                return golden_session.authenticate_token(conn, token)
        except golden_session.BusinessTokenInvalid as exc:
            # Audited in its own transaction: the refusal is the interesting
            # part, and the presented value is never stored.
            try:
                with connection(settings) as audit_conn:
                    golden_session.record_rejection(audit_conn, token=token, reason=exc.reason)
            except Exception:  # pragma: no cover - auditing must not mask the refusal
                logger.warning("golden token rejection could not be audited")
            raise _HTTPError(401, "invalid_business_token", str(exc))

    @app.post(f"{API_PREFIX}/golden/session/revoke")
    def golden_session_revoke(authorization: str | None = Header(default=None)) -> JSONResponse:
        """Revoke every live business token for this identity.

        Called on sign-out: after this returns, a token captured before the
        sign-out is refused, so "Clerk signed out" and "golden token accepted"
        cannot both hold. The Clerk session is used when it is still valid;
        when the browser has already lost it, the live business token itself is
        accepted as the proof — otherwise an out-of-order sign-out would leave
        ...
        token = _bearer_token(authorization)
        if not token:
            raise _HTTPError(401, "not_authenticated", "a clerk session or business token is required")

        try:
            user = _clerk_user(settings, authorization)
        except _HTTPError:
            user = None
        if user is not None:
            user_id, issuer, subject = _golden_identity(user)
            ...
            return JSONResponse(
                content={"revoked": revoked, "role": golden_session.BUSINESS_ROLE, "basis": "clerk_session"}
            )

        if not golden_session.looks_like_token(token):
            # A credential that cannot be one of ours is refused before any
            # database work: nothing is looked up, nothing is revoked.
            raise _HTTPError(401, "invalid_business_token", "a business token is required")
        try:
            with connection(settings) as conn:
                row = golden_session.authenticate_token(conn, token)
                revoked = golden_session.revoke_subject(
                    conn, subject_id=row["user_id"], reason="sign_out"
                )
        except golden_session.BusinessTokenInvalid as exc:
            raise _HTTPError(401, "invalid_business_token", str(exc))
        return JSONResponse(
            content={"revoked": revoked, "role": golden_session.BUSINESS_ROLE, "basis": "business_token"}
        )
