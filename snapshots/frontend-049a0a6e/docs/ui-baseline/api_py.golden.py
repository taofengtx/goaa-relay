...
def chat(req: ChatReq):
    ...
    if req.enable_tools:
        tools = [{
            ...
            "function": {
                "name": "dispatch_task",
                "description": "派發新任務給指定 worker 執行 (V5.1 新增, 對齊 dispatch_task backend). 用於用戶要求『派 health_check 給 aika-core-01』『執行 ping_test』等真實任務指派. 任務會進入 task_pool, worker 端會輪詢領取執行.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "task_type": {
                            "type": "string",
                            "description": "任務類型, 白名單: health_check / ollama_status / docker_status / system_status / ping_test / log_summary",
                            "enum": ["health_check", "ollama_status", "docker_status", "system_status", "ping_test", "log_summary"]
                        },
                        "target_worker": {
                            "type": "string",
                            "description": "目標 worker_id (預設 aika-core-01). 可選: aika-1, aika-2, aika-core-01, do-cloud-1, do-cloud-2, do-cloud-3",
                            "default": "aika-core-01"
                        },
                        "prompt": {
                            "type": "string",
                            "description": "任務描述 (可選, 例如 '檢查 aika-core-01 健康狀態')",
                            "default": ""
                        },
                        "priority": {
                            "type": "integer",
                            "description": "優先級 1-3 (1=高, 2=中, 3=低), 預設 2",
                            "default": 2
                        }
                    },
                    "required": ["task_type"]
    ...
    if req.enable_tools and tool_calls:
        ...
        for tc in tool_calls:
            ...
            if fn_name == "task_status":
                ...
                # V5.1.B: dispatch_task LLM tool (規範 #36 v2 對齊 backend L366-420)
                # 規範 #28 白名單檢查 (對齊 tasks_by_type pattern)
                _DISPATCH_ALLOWED = {"health_check", "ollama_status", "docker_status", "system_status", "ping_test", "log_summary"}
                _task_type = str(args.get("task_type", "")).strip().lower()
                _target_worker = str(args.get("target_worker", "aika-core-01")).strip()
                _prompt = str(args.get("prompt", ""))
                _priority = int(args.get("priority", 2))
                
                if not _task_type:
                    result = json.dumps({"error": "dispatch_task: missing required parameter 'task_type'"})
                elif _task_type not in _DISPATCH_ALLOWED:
                    result = json.dumps({"error": f"dispatch_task: task_type '{_task_type}' not allowed. Allowed: {sorted(_DISPATCH_ALLOWED)}"})
                elif _target_worker not in WORKER_REGISTRY:
                    result = json.dumps({"error": f"dispatch_task: unknown target_worker '{_target_worker}'. Known: {sorted(WORKER_REGISTRY.keys())}"})
                else:
                    try:
                        # 構造 DispatchReq (對齊 L267-284 Pydantic schema)
                        _dispatch_req = DispatchReq(
                            task_type=_task_type,
                            prompt=_prompt or f"LLM dispatched {_task_type} to {_target_worker}",
                            priority=_priority,
                            risk_level=1,
                            target_worker=_target_worker,
                            session_id=req.session_id,
                            submitted_by="chat_llm"
                        )
                        _dispatch_result = dispatch_task(_dispatch_req)
                        # dispatch_task 返回完整 task dict, 提取關鍵 fields 給 LLM 看
                        result = json.dumps({
                            "ok": True,
                            "task_id": _dispatch_result.get("task_id"),
                            "task_type": _task_type,
                            "target_worker": _target_worker,
                            "status": _dispatch_result.get("status", "pending"),
                            "message": f"任務已派發給 {_target_worker}, task_id={_dispatch_result.get('task_id')}, 預計幾秒內 worker 領取並執行. 請稍後用 tasks_by_type 查詢結果."
                        }, default=str, ensure_ascii=False)
                    except Exception as e:
                        result = json.dumps({"error": f"dispatch_task failed: {type(e).__name__}: {e}"})
            else:
